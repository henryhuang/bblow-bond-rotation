    ...
    return new Promise((resolve) => {
      ...
      axios({
        ...
        if (response.data.rows) {
          ...
          if (items.length < 100) {
            throw new Error('需要更新cookie，先登陆https://www.jisilu.cn/data/cbnew/cb_list/?___jsl=LST___t然后通过devtool去查看请求的cookie')
          }
          // 获取中间值
          ...
          // 过滤赎回日期在3周之内的
          const maxSpan = 3 * 7 * 24 * 60 * 60 * 1000
          items = items.filter(item => {
            if (item.redeemDate) {
              return moment(item.redeemDate).diff(date) > maxSpan
            }
            return true
          })
          this.logger.log(`过滤3周之内强赎，还剩${items.length}个基金`)
