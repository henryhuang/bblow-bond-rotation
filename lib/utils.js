...
const getLatestData = () => {
  ...
  fs.readdirSync(path.join(__dirname, '..', 'data'))
      .filter((file) => !fs.lstatSync(path.join(__dirname, '..', 'data', file)).isDirectory() && path.extname(file) === '.csv')
      .forEach(file => {
...
const isDailyLimited = (stock_id, increase_rt) => {
  if (stock_id.startsWith('30')) {
    return increase_rt >= 19.95
  }
  return increase_rt >= 9.95
}
