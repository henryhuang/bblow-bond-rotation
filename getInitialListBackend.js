...
const findMidPrice = (items) => {
  return items[parseInt(items.length * 0.5)].price
}

const printItems = (items) => {
  items.forEach(item => {
    // str = str + item.bond_id + '/' + item.bond_nm + '/' + item.dblow  + ', '
    // str = str + item.bond_nm + ', '
    console.log(item.bond_nm)
  })
}
